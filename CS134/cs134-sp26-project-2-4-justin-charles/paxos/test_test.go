...
func TestManyForget(t *testing.T) {
	...
	for seq := 0; seq < maxseq; seq++ {
		for i := 0; i < npaxos; i++ {
			if seq >= pxa[i].Min() {
				...
				if fate == Decided && v != proposed[seq] {
					t.Fatalf("decided value %v is not the proposed value %v", v, proposed[seq])
				}
			}
		}
	}
	...
	check_seq := maxseq + 1
	check_val := rand.Int()
	pxa[0].Start(check_seq, check_val)
	waitn(t, pxa, check_seq, npaxos)
	fate, v := pxa[0].Status(check_seq)
	if fate != Decided || v != check_val {
		t.Fatalf("Start/Status check failed, got %v expected %v", v, check_val)
	}
...
func TestMany(t *testing.T) {
	...
	for seq := 1; seq < ninst; seq++ {
		fate, v := pxa[0].Status(seq)
		if fate != Decided {
			t.Fatalf("instance %v not decided", seq)
		}
		valid := false
		for i := 0; i < npaxos; i++ {
			if v == (seq*10)+i {
		...
		if !valid {
			t.Fatalf("decided value %v is not one of the proposed values", v)
		}
	}
...
func TestManyUnreliable(t *testing.T) {
	...
	const ninst = 500 // https://piazza.com/class/mmb7twnyzbt64d/post/101
	for seq := 1; seq < ninst; seq++ {
