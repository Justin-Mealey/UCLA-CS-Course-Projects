...
func TestConcurrentSame(t *testing.T) {
	...
	var mu sync.Mutex
	validVals := make(map[string]map[string]bool)
	for i := 0; i < 2; i++ {
		validVals[strconv.Itoa(i)] = make(map[string]bool)
	}
	...
	for i := 0; i < nkeys; i++ {
		...
		mu.Lock()
		valid := validVals[strconv.Itoa(i)][vals[i]]
		mu.Unlock()
		if !valid {
			t.Fatalf("Get(%v) returned a value not generated by clients: %v", i, vals[i])
		}
	}
