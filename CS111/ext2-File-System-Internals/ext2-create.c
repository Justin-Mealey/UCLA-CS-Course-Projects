...
void write_superblock(int fd) {
	...
	superblock.s_blocks_per_group = NUM_BLOCKS * 8; //follows dumpe2fs output
...
void write_block_bitmap(int fd)
{
	...
	u8 map_value[BLOCK_SIZE] = {0}; //this is accounting for BLOCK_SIZE*8 = 8192 blocks

	for(u32 i = 1; i <= LAST_BLOCK; i++){ 
		u32 index = (i - 1) / 8;
		u32 bit_pos = (i - 1) % 8; //bit of the u8 at index
		map_value[index] |= (1 << bit_pos);
	}

	u32 num_bits_in_map = NUM_BLOCKS * 8;
	for(u32 i = NUM_BLOCKS; i <= num_bits_in_map; i++){ //padding for extra blocks we can't access
		u32 index = (i - 1) / 8;
		u32 bit_pos = (i - 1) % 8;
		map_value[index] |= (1 << bit_pos);
	}
...
void write_inode_bitmap(int fd)
{
	...
	for(u32 i = 1; i <= LAST_INO; i++){ //bits 0-12 set
		u32 index = (i - 1) / 8;
		u32 bit_pos = (i - 1) % 8;
		map_value[index] |= (1 << bit_pos);
	}
	
	u32 total_inodes_in_map = NUM_BLOCKS * 8;
	for (u32 i = NUM_INODES + 1; i <= total_inodes_in_map; i++) { //padding for bits 128 on
		u32 index = (i - 1) / 8;
		u32 bit_pos = (i - 1) % 8;
		map_value[index] |= (1 << bit_pos);
    }
...
void write_inode_table(int fd) {
	...
	hello_world_inode.i_links_count = 1; //symlink doesn't count, so only link is our original 
	...
	hello_inode.i_size = 11; //"hello-world" is 11 characters
